beta.parms.from.quantiles <- function(
  q,
  p = c(0.025, 0.975),
  precision = 0.001,
  ...
  F.inv <- function(x, theta) {
    qbeta(x, shape1 = theta[1], shape2 = theta[2])
  }
  f.cum <- function(x, theta) {
    pbeta(x, shape1 = theta[1], shape2 = theta[2])
  }
  f.mode <- function(theta) {
    a <- theta[1]
    b <- theta[2]
    mode <- ifelse(a > 1, (a - 1) / (a + b - 2), NA)
    mode
  }
  theta.from.moments <- function(m, v) {
    a <- m * m * (1 - m) / v - m
    b <- a * (1 / m - 1)
    c(a, b)
  }
  ...
  print.area.text <- function(
    ...
    w <- which(
      (par.usr[1] + str.width[1]) <= x & (y + str.height) <= par.usr[4]
    )
    x <- x[w]
    y <- y[w]
    ...
    mass.center <- c(
      mean(tmp.x),
      sum(h[-length(h)] * diff(tmp.x)) / diff(range(tmp.x))
    )
    ...
    # Eliminate points for which the text would fall out of the plot area
    w <- which(
      (par.usr[2] - str.width[2]) >= x & (y + str.height) <= par.usr[4]
    )
    x <- x[w]
    y <- y[w]
  ...
  Newton.Raphson <- function(
    ...
    while (max(abs(change)) > precision) {
      Hessian[, 1] <- (f.cum(q, theta) -
        f.cum(q, theta - c(derivative.epsilon, 0))) /
        derivative.epsilon
      Hessian[, 2] <- (f.cum(q, theta) -
        f.cum(q, theta - c(0, derivative.epsilon))) /
        derivative.epsilon
  ...
  plot.density <- function(
    ...
    points(
      q,
      rep(par('usr')[3] + 0.15 * par('cxy')[2], 2),
      pch = 17,
      col = 'orange'
    )
  }
